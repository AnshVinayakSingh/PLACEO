...
export function InterviewCallRoom({
  ...
  const analyzeCandidateTurn = useCallback(async (question: string, answer: string) => {
    ...
    try {
      ...
      if (payload?.analysis) {
        setLiveAssessment(payload.analysis)
        // Same two-strike engine as the proctor: one joke/nonsense answer gets a
        // warning, a second one (or a second integrity issue of any kind) ends
        // the interview. This is a signal, not certainty, so the check stays
        // conservative server-side (see live-analysis/route.ts).
        if (payload.analysis.isNonsenseOrJoke) {
          strike(payload.analysis.nonsenseReason || 'That answer did not look like a genuine attempt to answer the question')
        }
      }
    } catch {
